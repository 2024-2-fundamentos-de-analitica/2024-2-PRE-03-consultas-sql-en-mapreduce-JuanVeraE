...
def _load_input(input_directory):
    sequence = []
    files = glob.glob(f"{input_directory}/*") #Crea listado de archivos
    with fileinput.input(files=files) as f:
        for line in f:
            sequence.append((fileinput.filename(), line)) #Tupla con el nombre de archivo y la linea
    return sequence
...
def _create_ouptput_directory(output_directory):
    if os.path.exists(output_directory):
        for file in glob.glob(f"{output_directory}/*"):
            os.remove(file)
        os.rmdir(output_directory) # Borra la carperta sin archivos viejos
    os.makedirs(output_directory) # Crea carpeta vacia

def _create_marker(output_directory):
    """Create Marker"""
    with open(f"{output_directory}/_SUCCESS", "w", encoding="utf-8") as f: # Si lo abre retorna SUCCESS
        f.write("")
